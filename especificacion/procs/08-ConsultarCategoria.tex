...
\begin{proc}{AcumularMillas}
	...
	\aseguraLargo{
		...
		\\
		\hyperref[sec:MillasCalculadas]{MillasCalculadas}(distancia, \hyperref[sec:DarFactorCategoria]{DarFactorCategoria}(AM.Historial,idUsuario),
		...
		AM0.Promociones[nombrePromo].millasRestantes - \hyperref[sec:MillasCalculadas]{MillasCalculadas}\\(distancia, \hyperref[sec:DarFactorCategoria]{DarFactorCategoria}(AM.Historial, idUsuario), \\
		...
		\\\hyperref[sec:DarFactorCategoria]{DarFactorCategoria}(AM0.Historial,idUsuario),
		...
		\\
		\hyperref[sec:DarCategoria]{DarCategoria}(AM.Historial, idUsuario)) \land
		\\
...
\comentario{Falta implementar el predicado TransaccionCanje.}
\begin{proc}{CanjearMillas}
	...
	\aseguraLargo{
		\IfThenElse{ \hyperref[sec:MillasDisponibles]{MillasDisponibles}(AM_{0}.Historial, IdUsuario) \geq MillasCanjeadas}
...
\begin{proc}{TransferirMillas}
	...
	\aseguraLargo{\hyperref[sec:EsNuevaTransaccion]{EsNuevaTransaccion}( AM, AM0, idUsuarioPagador, idUsuarioBeneficiario,1, CantMillasTrans, NoPromo, 1) \land
		\land AM.Promociones = AM0.Promociones \\
		\land AM.Usuarios = AM0.Usuarios
		\land \hyperref[sec:SubioCategoria]{SubioCategoria}(idUsuarioBeneficiario)
	}
...
\begin{proc}{RankingMillas}
	...
	\aseguraLargo{ (|AM.Usuarios| > n  \land_L |res| = n) \\ \lor (|AM.Usuarios| \leq n  \land_L |res| = |AM.Usuarios|)
		...
		\\ \land_L \hyperref[sec:ordenDecreciente]{ordenDecreciente}(AM.HistorialDeTransacciones, res)
		\\ \land_L \hyperref[sec:losMejoresCandidato]{losMejoresCandidato}(AM.Usuarios, AM.
		HistorialDeTransacciones, res)
	}
\end{proc}
...
\begin{proc}{ResumenTransacciones}
	...
	\aseguraLargo{ res_{0} = \hyperref[sec:MillasObtenidasPorVuelo]{MillasObtenidasPorVuelo}(AM.HistorialDeTransacciones, id) \yLuego
		\\
		res_{1} = \hyperref[sec:MillasDescontadasPorCanjeo]{MillasDescontadasPorCanjeo}(AM.HistorialDeTransacciones, id) \yLuego
		\\
		res_{2} = MillasTransferidasDelUsuario(AM.HistorialDeTransacciones, id)
	}
\end{proc}
...
\requiereLargo{
	...
	claves(AM0.Promociones) \land TopePromo > 0
}
\aseguraLargo{
	AM.Usuarios = AM0.Usuarios \land
	\\
...
\begin{proc}{FinalizarPromocion}
	...
	\requiereLargo{
		...
		\\
		nombrePromo \in claves(AM0.promociones)
	}
	\aseguraLargo{
		AM.Usuarios = AM0.Usuarios \land
		\\
...
\begin{proc}{ElQueTieneMasMillas}
	...
	\requiere{ \norm{AM.Usuarios} > 0}
	\aseguraLargo{
		\existe{i}{\Tipo{\Z}}{i \in claves(AM.Usuarios) \yLuego \hyperref[sec:MillasTotales]{MillasTotales}(AM.Historial,i) = res} \land
		\\
		\paraTodo{j}{\Tipo{\Z}}{j \in claves(AM.Usuarios) \implicaLuego \hyperref[sec:MillasTotales]{MillasTotales}(AM.Historial,j) \leq res}
	}
\end{proc}
...
\begin{proc}{paresTransferidoresExclusivos}
		...
		\aseguraLargo{
            ...
            \\
            \hyperref[sec:NoHayRepetidos]{NoHayRepetidos}(res) \land_L
            \\
            \hyperref[sec:NoHayPermutacion]{NoHayPermutacion}(res) \land_L
            \\
            \hyperref[sec:TieneReciprocoHistorial]{TieneReciprocoHistorial}(AM.HistorialDeTransacciones, res) \land_L
            \\
            \hyperref[sec:SonParejaExclusiva]{SonParejaExclusiva}(AM.HistorialDeTransacciones, res) \land_L
            \\
            EstanTodosLosParesExclusivos(
                AM.Usuarios,
                AM.HistorialDeTransacciones,
                res
            )
        }
	\end{proc}
...
\\
\phantomsection\hypertarget{sec:losMejoresCandidato}{}\label{sec:losMejoresCandidato}
...
\phantomsection\hypertarget{sec:MillasTransferidas}{}\label{sec:MillasTransferidas}
...
{\Tipo{\R}}
{
	\sum\limits_{i=0}^{\norm{Historial} - 1}
	\IfThenElse{
		\hyperref[sec:EsTransferencia]{EsTransferencia}(Historial[i],
		\\
		idOrigen, idDestino)
	}{
		Historial[i].Millas
...
\phantomsection\hypertarget{sec:EsTransferencia}{}\label{sec:EsTransferencia}
