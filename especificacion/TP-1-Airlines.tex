...
\begin{proc}{AcumularMillas}
	...
	\aseguraLargo{
		...
		\\
		SeConsumioLaPromocion(nombrePromo, AM, AM0)
	}
\end{proc}
...
\begin{proc}{TransferirMillas}
	...
	\requiere{AM0 = AM \land idUsuarioPagador \in claves(AM.usuarios)\\
		...
		\land MillasDisponibles(AM, idUsuarioPagador) \geq CantMillasTrans
	}
	\aseguraLargo{EsNuevaTransaccion( AM, AM0, idUsuarioPagador, idUsuarioBeneficiario,1, CantMillasTrans, NoPromo, 1) \land
		\land AM.Promociones = AM0.Promociones \\
		\land AM.Usuarios = AM0.Usuarios
		\land SubioCategoria(idUsuarioBeneficiario)
	}
...
\begin{proc}{ConsultarSaldo}
	...
	\aseguraLargo{
		res = MillasDisponibles(IdUsuario,AM.Historial)
	}
\end{proc}
...
\begin{proc}{RankingMillas}
	...
	\aseguraLargo{ (|AM.Usuarios| > n  \land_L |res| = n) \\ \lor (|AM.Usuarios| \leq n  \land_L |res| = |AM.Usuarios|)
		...
		\\ \land_L ordenDecreciente(AM.HistorialDeTransacciones, res)
		\\ \land_L losMejoresCandidato(AM.Usuarios, AM.
...
\begin{proc}{ResumenTransacciones}
	...
	\aseguraLargo{ res_{0} = MillasObtenidasPorVuelo(AM.HistorialDeTransacciones, id) \yLuego
		\\
		res_{1} = MillasDescontadasPorCanjeo(AM.HistorialDeTransacciones, id) \yLuego
		\\
...
\begin{proc}{paresTransferidoresExclusivos}
		...
		\aseguraLargo{
            ...
            \\
            NoHayRepetidos(res) \land_L
            \\
            NoHayPermutacion(res) \land_L
            \\
            TieneReciprocoHistorial(AM.HistorialDeTransacciones, res) \land_L
            \\
            SonParejaExclusiva(AM.HistorialDeTransacciones, res) \land_L
            \\
...
{\R}
{
	MillasCalculadas(distancia,factorCategoria,\\
...
\label{sec:EsTransferenciaEnviada}
